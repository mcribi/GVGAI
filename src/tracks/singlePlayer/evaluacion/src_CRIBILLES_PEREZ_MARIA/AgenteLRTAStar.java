...
public class AgenteLRTAStar extends AbstractPlayer {
		...
		public ACTIONS alg_rta(StateObservation stateObs, ElapsedCpuTimer elapsedTimer, Vector2d posInicial, Vector2d posFinal){
			...
			for (Nodo sucesor : sucesores) { //iteramos por cada sucesor
			    if (sucesor.f < mejorF) { //si el coste es menor que el mejor
			        mejorF = sucesor.f; //actualizamos el mejor
			        mejorSucesor = sucesor; //actualizamos el mejor sucesor
			    }
			}
			
			//en este algoritmo no se considera el segundo mejor, solo el primer mejor sucesor
			...
			//guardamos en tabla y actualizamos nodo
			tablaHeuristicas.put(claveActual, nuevaHeuristica);
			nodo_actual.heuristica = nuevaHeuristica;
			nodo_actual.f = nuevaHeuristica + nodo_actual.coste;  //f=g+h
